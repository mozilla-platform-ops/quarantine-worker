...
if [ -f "${GOPATH}/bin/quarantine-worker" ]; then
  mkdir "${GOPATH}/bin/linux_amd64"
  mv "${GOPATH}/bin/quarantine-worker" "${GOPATH}/bin/linux_amd64/quarantine-worker"
fi
...
export RELEASE_FILE="${TRAVIS_BUILD_DIR}/quarantine-worker-${GIMME_OS}-${GIMME_ARCH}${FILE_EXT}"
mv "${GOPATH}/bin/${GIMME_OS}_${GIMME_ARCH}/quarantine-worker${FILE_EXT}" "${RELEASE_FILE}"
